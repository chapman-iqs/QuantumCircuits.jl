...
begin
	exportpath = let
					ep = string("animations/", Dates.now())
					replace(ep, ":" => "-") end
...
# ╔═╡ 4e4e059a-8c92-4ef9-a03f-3c1ffd819696
begin
	const Time = Float64
	const Rate = Float64
...
let
	...
	H(t::Time, r::Record) = (ΔS/2) * r[1] * σz + (ΩR / 2) * σy
	J = [(σz, ((1-η)*Γm))]
	C = [(im * σz, Γm, η)]
...
let
	...
	Δ0 = 0.0 #ideal ? -sin(2θs)/(4τm) : -sin(2θs)/(4τm*Rs^2) 
	Δ1 = 0.0 # ideal ? sin(θs)/τm : sin(θs)/(τm*Rs)
	...
	H = 0.0 * id
	# H(t::Time, r::Record) = 0.0 * id # (Δ0 + Δ1*r[1]) * σϕ/2
...
let
	...
	# loop over N simulations
	for i in 1:N
		sol = fbayesian(T, ρ0, H, J, C; dt=dt, td=td)
		tr = traj(sol)
		push!(trajs, tr)
	end
