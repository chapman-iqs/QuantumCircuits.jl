...
begin
	Random.seed!(1)
	sol_ens = ensemble(bayesian, (0,4τ0), ρ0, H0, J0, C0; dt=dt, N=500)
end
...
# ╔═╡ f090f515-e4fa-4078-9b0d-464ed710347c
begin
	Random.seed!(1)
	sol_ensr = ensemble(rouchon, (0,4τ0), ρ0, H0, J0, C0; dt=dt, N=500)
end
...
η = 1 | - - - - - - | Γ = 0.05
$(@bind Γ html"<input type=range min=0.05 max=0.25 step=0.05 value=0.15>")
Γ = 0.25
...
md"""
ΩR = $(round(ΩR/π, digits=3)) π rad MHz | - - - - - - | η = $η | - - - - - - | Γ = $Γ MHz | - - - - - - | τ = 1/(2Γ) = $(round(τ,digits=3)) μs
"""

# ╔═╡ 09b609f6-a441-4ae2-aaff-66c1b8b6ffae
begin
	Random.seed!(1)
	sol2 = bayesian((0, 2τ0), ρ0, H, [J], [C]; dt=dt)
end
...
begin
	title_string = "bayesian homodyne; Γ = $(Γ) MHz, η = $(η)"
	plot_solution(sol2; plot_title=title_string, xlab=L"$t$  (μs)")
end
...
# ╔═╡ 227b47e4-4454-4b40-a1d0-66505ad2c19a
 plot_ensemble(sol_ensr)

# ╔═╡ 155a469e-0b5b-48e2-a5c5-8be09dbd0ed8
 plot_ensemble(sol_ensr, average=true)
...
if Γ > Γ0
	green(md"""As the the measurement-induced dephasing rate $\Gamma = 1/(2\tau)$ decreases, the system takes longer to dephase (remains high purity).""", title="τ increased from 3.0")
	
elseif Γ < Γ0
	red(md"""As the measurement-induced dephasing rate $\Gamma = 1/(2\tau)$ increases, the system dephases quickly and the purity decreases.""", title="τ decreased from 3.0")
