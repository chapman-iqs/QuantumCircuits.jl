...
begin
	path_ket = string("test/purestate_test_data/", "2022-01-15T15-12-59.746-ket-from-record", "/")
	path_dm = string("test/purestate_test_data/", "2022-01-15T14-52-32.301-dm", "/")
end
...
begin
	(pars_ket, trajs_ket, ens_avg_ket) = load(path_ket)
	(pars_dm, trajs_dm, ens_avg_dm) = load(path_dm)
end
...
begin
	sim = trajs_ket[i]
	blochtimeseries(sim.t, sim.x, sim.y, sim.z, size=(800,300), legend=:outerright, title=string("trajectory ", i, ", pure state"))
end

# ╔═╡ 212d44fe-6066-4668-b6b3-e2fa1993f8bd
trajs_ket.ρ
...
plotresults(trajs_ket[i], trajs_dm[i])
